...
class _MapScreenState extends State<MapWidget> {
  ...
  void initState() {
    ...
    );// Initialize with the passed activeSteep
    getCureentLocation();
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                NextButton('Next', const Bookingcomplete(), () {}, 250)
              ],
            ),
          ),
        ),
  ...
  Future<void> getCureentLocation() async {
    ...
    try {
      ...
      setState(() {
        ...
        _addNeighboringMarkers(userLocation.latitude!, userLocation.longitude!);
      });
    } on Exception {
  ...
  Future<void> _addNeighboringMarkers(double lat, double lon) async {
    // Dummy data for neighboring places
    final neighbors = [
      LatLng(lat + 0.01, lon),  // North neighbor
      LatLng(lat - 0.01, lon),  // South neighbor
      LatLng(lat, lon + 0.01),  // East neighbor
      LatLng(lat, lon - 0.01),  // West neighbor
    ];

    setState(() {
      markers.addAll(neighbors.map((neighbor) => Marker(
        width: 80.0,
        height: 80.0,
        point: neighbor,
